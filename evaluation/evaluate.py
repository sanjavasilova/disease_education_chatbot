...
Usage:
    python3 -m evaluation.evaluate --retrieval-only
    python3 -m evaluation.evaluate --limit 3
    # Cheap LLM-as-judge on existing retrieval results (~10 API calls for sample=5):
    python3 -m evaluation.evaluate --augment-results evaluation/results.json \\
        --judge-sample 5 --judge-metrics correctness
    # Recommended for statistical auto↔judge comparison (resume-safe):
    python3 -m evaluation.evaluate --augment-results evaluation/results_with_judge.json \\
        --judge-sample 25 --judge-metrics context_relevance,correctness \\
        --output evaluation/results_with_judge.json
    python3 -m evaluation.statistical_analysis auto-vs-judge evaluation/results_with_judge.json \\
...
def run_evaluation(
    ...
    out = Path(output_path) if output_path else (_root / "evaluation" / "results.json")
    out.write_text(json.dumps(results, indent=2, ensure_ascii=False), encoding="utf-8")
